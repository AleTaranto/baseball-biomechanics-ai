...
class FrameExtractionResponse(BaseModel):
    ...
    width: int | None = Field(
        default=None, ge=1, description="Source video width in pixels, when available."
    )
    height: int | None = Field(
        default=None, ge=1, description="Source video height in pixels, when available."
    )
    duration_seconds: float | None = Field(
        default=None,
        description=(
            "Source video duration in seconds when available from metadata or "
            "frame timing."
        ),
    )
