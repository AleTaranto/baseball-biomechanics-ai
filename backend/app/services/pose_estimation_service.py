...
class PoseEstimationService:
    ...
    def estimate_video(
        ...
        video_id: str,
        *,
        frame_entries: list[dict[str, object]] | None = None,
        force: bool = False,
    ) -> PoseEstimationResponse:
        manifest_path = self._pose_manifest_path(video_id)
        if not force and manifest_path.exists():
            return self.load_manifest(video_id)
        ...
        selected_frames = (
            frame_entries if frame_entries is not None else frame_manifest.get("frames", [])
        )
        ...
        for frame_entry in selected_frames:
            frame_index = int(cast(int, frame_entry["frame_index"]))
            timestamp_ms = int(cast(int, frame_entry["timestamp_ms"]))
            relative_path = str(cast(str, frame_entry["relative_path"])).replace("\\", "/")
