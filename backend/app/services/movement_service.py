        ...
        for frame_result in response.frames:
            ...
            frames.append(
                FramePose(
                    ...
                    timestamp_seconds=frame_result.timestamp_ms / 1000.0,
                    detected=frame_result.detected,
                    joints=joints,
                )
            )

        duration_seconds = max((frame.timestamp_seconds for frame in frames), default=0.0)
...
class PoseSequenceValidator:
    def __init__(
        ...
        *,
        ...
        max_temporal_gap_seconds: float = 0.2,
    ) -> None:
        self.low_confidence_threshold = float(low_confidence_threshold)
        self.max_temporal_gap_seconds = float(max_temporal_gap_seconds)
    ...
    def validate(self, recording: MovementRecording) -> PoseSequenceValidationResult:
        ...
        for frame in sorted_frames:
            ...
            if previous_timestamp is not None and frame.timestamp_seconds < previous_timestamp:
                issues.append(
                    PoseSequenceIssue(
                        code="timestamp-order",
                        message=(
                            f"Timestamp {frame.timestamp_seconds:.6f} s is out of order after "
                            f"{previous_timestamp:.6f} s."
                        ),
                        frame_index=frame.frame_index,
                    )
                )
            if previous_timestamp is not None:
                delta = frame.timestamp_seconds - previous_timestamp
                if delta > self.max_temporal_gap_seconds:
                    issues.append(
                        PoseSequenceIssue(
                            code="temporal-gap",
                            message=(
                                "Temporal discontinuity detected between consecutive "
                                f"frames: {delta:.6f} s exceeds threshold "
                                f"{self.max_temporal_gap_seconds:.6f} s."
                            ),
                            frame_index=frame.frame_index,
                        )
                    )
...
class MovementDataService:
    ...
    @staticmethod
    def load_recording(video_id: str, *, base_dir: str | Path | None = None) -> MovementRecording:
        root = Path(base_dir) if base_dir is not None else Path(__file__).resolve().parents[3]
        movement_dir = root / "sample-data" / "movement"
        target = movement_dir / f"{video_id}.json"
        if not target.exists():
            raise ValueError(f"No movement recording exists for video id '{video_id}'.")
        payload = json.loads(target.read_text(encoding="utf-8"))
        return MovementRecording.model_validate(payload)
