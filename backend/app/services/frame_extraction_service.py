...
class FrameExtractionService:
    ...
    def extract_frames(self, video_id: str) -> FrameExtractionResponse:
        ...
        try:
            source_width = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH)) or None
            source_height = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT)) or None
            fps_value = capture.get(cv2.CAP_PROP_FPS)
            fps: float | None = float(fps_value) if fps_value and fps_value > 0 else None
            frame_count_value = capture.get(cv2.CAP_PROP_FRAME_COUNT)
            frame_count = (
                int(frame_count_value) if frame_count_value and frame_count_value > 0 else None
            )
            ...
            if fps is not None and fps > 0 and frame_count is not None and frame_count > 0:
                ...
                duration_seconds = float(len(extracted_frames)) / fps
            else:
                duration_seconds = (
                    extracted_frames[-1].timestamp_ms / 1000.0
                    if extracted_frames
                    else None
                )
